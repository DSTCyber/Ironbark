...
subsubsection \<open>Decomposition Rules Over SHIFT\_LEFT\<close>

theory SHIFT_LEFT_decomposition
...
of decomposing the \<open>SHIFT_LEFT\<close> instruction. 
...
lemma (in Ironbark_world) SHIFT_LEFT_decomp_state:
  ...
  shows
    \<open>SHIFT_LEFT reg1 reg2 reg3 state = 
      state
      \<lparr>
        register_state := 
