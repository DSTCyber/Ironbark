...
subsubsection \<open>Decomposition Rules Over NOP\<close>

theory NOP_decomposition
...
decomposing the \<open>NOP\<close> instruction. The three lemmas are the `special' case where we 
...
lemma (in Ironbark_world) NOP_decomp_manipulation:
  assumes
    \<open>typical_flags state\<close>
  shows
    \<open>NOP state = standard_post_instruction common_instruction_duration state\<close>
  using assms
