...
subsubsection \<open>Decomposition Rules Over EQUALS\<close>

theory EQUALS_decomposition
...
of decomposing the \<open>EQUALS\<close> instruction. 
...
lemma (in Ironbark_world) EQUALS_decomp_state:
  assumes
  ...
  shows
    \<open>EQUALS reg1 reg2 reg3 state =
      state
      \<lparr>
        register_state :=
...
lemma (in Ironbark_world) EQUALS_decomp_mixed:
  assumes
    \<open>typical_flags state\<close>
    \<open>check_write_permission reg1\<close>
    \<open>check_read_permission reg2\<close>
