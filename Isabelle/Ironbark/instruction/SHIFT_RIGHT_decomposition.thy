...
subsubsection \<open>Decomposition Rules Over SHIFT\_RIGHT\<close>

theory SHIFT_RIGHT_decomposition
...
of decomposing the \<open>SHIFT_RIGHT\<close> instruction. 
...
lemma (in Ironbark_world) SHIFT_RIGHT_decomp_state:
  assumes
  ...
  shows
    \<open>SHIFT_RIGHT reg1 reg2 reg3 state = 
      state
      \<lparr>
        register_state := 
          ...
          (
            ...
            cycles_register_ref 
              \<mapsto> the (register_state state cycles_register_ref) + common_instruction_duration
          )
      \<rparr>\<close>
  using assms
...
lemma (in Ironbark_world) SHIFT_RIGHT_decomp_mixed:
  assumes
    \<open>typical_flags state\<close>
    \<open>check_write_permission reg1\<close>
    \<open>check_read_permission reg2\<close>
      ...
      \<lparr>
        register_state := 
          ...
          (
            ...
            cycles_register_ref 
              \<mapsto> (read_register cycles_register_ref state) + common_instruction_duration
          )
      \<rparr>\<close>
  using assms
  apply(simp add: instruction_impl_defs)
