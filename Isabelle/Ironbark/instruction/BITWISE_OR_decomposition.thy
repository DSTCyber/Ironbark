...
subsubsection \<open>Decomposition Rules Over BITWISE\_OR\<close>

theory BITWISE_OR_decomposition
...
of decomposing the \<open>BITWISE_OR\<close> instruction. 
...
lemma (in Ironbark_world) BITWISE_OR_decomp_state:
  ...
  shows
    \<open>BITWISE_OR reg1 reg2 reg3 state = 
      state
      \<lparr>
        register_state := 
          ...
          (
            reg1 
              \<mapsto> the (register_state state reg2) OR the (register_state state reg3),
    ...
    \<open>BITWISE_OR reg1 reg2 reg3 state = 
      ...
      \<lparr>
        register_state := 
          ...
          (
            reg1 
              \<mapsto> (read_register reg2 state) OR (read_register reg3 state),
            last_instruction_pointer_ref 
