...
subsubsection \<open>Decomposition Rules Over LOAD\_INPUT\_DATA\<close>
...
decomposing the \<open>LOAD_INPUT_DATA\<close> instruction. The three lemmas are the `special' case 
...
lemma (in Ironbark_world) LOAD_INPUT_DATA_decomp_state:
  ...
  shows
    \<open>LOAD_INPUT_DATA reg1 reg2 state = 
      state
      \<lparr>
        register_state := 
          ...
          (
            reg1 
              \<mapsto> the (input_memory state time (the (register_state state reg2))),
    ...
    \<open>LOAD_INPUT_DATA reg1 reg2 state = 
      ...
      \<lparr>
        register_state := 
          ...
          (
            reg1 
              \<mapsto> read_input_memory (read_register reg2 state) state,
            last_instruction_pointer_ref 
              \<mapsto> read_register instruction_pointer_ref state,
            instruction_pointer_ref 
              \<mapsto> (read_register instruction_pointer_ref state) + 1,
