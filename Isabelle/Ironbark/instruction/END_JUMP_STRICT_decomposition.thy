...
subsubsection \<open>Decomposition Rules Over END\_JUMP\_STRICT\<close>

theory END_JUMP_STRICT_decomposition
...
decomposing the end jump strict instruction. The three lemmas are the `special' case 
...
lemma (in Ironbark_world) END_JUMP_STRICT_decomp_state:
  ...
  shows
    \<open>END_JUMP_STRICT immediate state = 
      state
      \<lparr>
