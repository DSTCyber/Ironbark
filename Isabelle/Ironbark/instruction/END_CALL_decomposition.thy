...
subsubsection \<open>Decomposition Rules Over END\_CALL\<close>

theory END_CALL_decomposition
...
of decomposing the \<open>END_CALL\<close> instruction. 
...
lemma (in Ironbark_world) END_CALL_decomp_state:
  ...
  shows
    \<open>END_CALL state = 
      state
      \<lparr>
        flag_state := (flag_state state)\<lparr>end_call := 0\<rparr>,
        register_state := 
