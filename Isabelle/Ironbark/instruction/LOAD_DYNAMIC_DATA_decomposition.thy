...
subsubsection \<open>Decomposition Rules Over LOAD\_DYNAMIC\_DATA\<close>
...
decomposing the \<open>LOAD_DYNAMIC_DATA\<close> instruction. The three lemmas are the `special' 
...
lemma (in Ironbark_world) LOAD_DYNAMIC_DATA_decomp_state:
  ...
  shows
    \<open>LOAD_DYNAMIC_DATA reg1 reg2 state = 
      state\<lparr>
        register_state := 
    ...
    \<open>LOAD_DYNAMIC_DATA reg1 reg2 state = 
      ...
      \<lparr>
        register_state := 
          ...
          (
            reg1 
              \<mapsto> read_dynamic_memory (read_register reg2 state) state,
            last_instruction_pointer_ref 
              \<mapsto> read_register instruction_pointer_ref state,
            instruction_pointer_ref 
              \<mapsto> (read_register instruction_pointer_ref state) + 1,
