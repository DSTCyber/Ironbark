...
subsubsection \<open>Decomposition Rules Over BITWISE\_XOR\<close>

theory BITWISE_XOR_decomposition
...
of decomposing the \<open>BITWISE_XOR\<close> instruction. 
...
lemma (in Ironbark_world) BITWISE_XOR_decomp_state:
  ...
  shows
    \<open>BITWISE_XOR reg1 reg2 reg3 state = 
      state
      \<lparr>
        register_state := 
          ...
          (
            reg1 
              \<mapsto> the (register_state state reg2) XOR the (register_state state reg3),
    ...
    \<open>BITWISE_XOR reg1 reg2 reg3 state = 
      ...
      \<lparr>
        register_state := 
          ...
          (
            reg1 
              \<mapsto> (read_register reg2 state) XOR (read_register reg3 state),
            last_instruction_pointer_ref 
