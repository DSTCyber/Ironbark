...
subsubsection \<open>Simplification Rules Over BITWISE\_XOR\<close>

theory BITWISE_XOR_simps
...
bitwise xor instruction. The various rules are primarily 
...
lemma (in Ironbark_world) flag_BITWISE_XOR:
  ...
  shows
    \<open>flag_state (BITWISE_XOR reg1 reg2 reg3 state) = flag_state state\<close>
  using assms
  by(simp add: BITWISE_XOR_decomp_mixed)

lemma (in Ironbark_world) memory_BITWISE_XOR:
  shows
    \<open>program_memory (BITWISE_XOR reg1 reg2 reg3 state) = program_memory state\<close>
    \<open>call_memory    (BITWISE_XOR reg1 reg2 reg3 state) = call_memory    state\<close>
    \<open>static_memory  (BITWISE_XOR reg1 reg2 reg3 state) = static_memory  state\<close>
    \<open>dynamic_memory (BITWISE_XOR reg1 reg2 reg3 state) = dynamic_memory state\<close>
    \<open>input_memory   (BITWISE_XOR reg1 reg2 reg3 state) = input_memory   state\<close>
    \<open>output_memory  (BITWISE_XOR reg1 reg2 reg3 state) = output_memory  state\<close>
...
lemma (in Ironbark_world) read_flag_BITWISE_XOR:
  assumes
    ...
    \<open>check_read_permission reg3\<close>
  shows
    \<open>get_end_jump   (BITWISE_XOR reg1 reg2 reg3 state) = 0\<close>
    \<open>get_end_call   (BITWISE_XOR reg1 reg2 reg3 state) = 0\<close>
    \<open>get_end_return (BITWISE_XOR reg1 reg2 reg3 state) = 0\<close>
    \<open>get_halt       (BITWISE_XOR reg1 reg2 reg3 state) = 0\<close>
    \<open>get_error      (BITWISE_XOR reg1 reg2 reg3 state) = 0\<close>
  using assms
...
lemma (in Ironbark_world) common_flags_BITWISE_XOR:
  assumes
    ...
    \<open>check_read_permission reg3\<close>
  shows
    \<open>typical_flags (BITWISE_XOR reg1 reg2 reg3 state)\<close>
  using assms
...
lemma (in Ironbark_world) read_register_BITWISE_XOR_diff:
  ...
  shows
    \<open>read_register    regID (BITWISE_XOR reg1 reg2 reg3 state) 
...
lemma (in Ironbark_world) read_register_BITWISE_XOR_same:
  ...
  assumes
    ...
    \<open>check_read_permission reg3\<close>
  shows
    \<open>read_register reg1 (BITWISE_XOR reg1 reg2 reg3 state) 
    = read_register reg2 state XOR read_register reg3 state\<close>
  using assms
...
lemma (in Ironbark_world) read_ip_BITWISE_XOR:
  assumes
    ...
    \<open>check_read_permission reg3\<close>
  shows
    \<open>read_register instruction_pointer_ref (BITWISE_XOR reg1 reg2 reg3 state) 
...
lemma (in Ironbark_world) read_cycles_BITWISE_XOR:
  assumes
    ...
    \<open>check_read_permission reg3\<close>
  shows
    \<open>read_register cycles_register_ref (BITWISE_XOR reg1 reg2 reg3 state) 
...
lemma (in Ironbark_world) read_last_ip_BITWISE_XOR:
  assumes
    ...
    \<open>check_read_permission reg3\<close>
  shows
    \<open>read_register last_instruction_pointer_ref (BITWISE_XOR reg1 reg2 reg3 state) 
...
lemma (in Ironbark_world) read_memory_BITWISE_XOR:
  shows
    \<open>read_program_memory  address (BITWISE_XOR reg1 reg2 reg3 state) 
    = read_program_memory address state\<close>

    \<open>read_call_memory     address (BITWISE_XOR reg1 reg2 reg3 state) 
    = read_call_memory    address state\<close>

    \<open>read_static_memory   address (BITWISE_XOR reg1 reg2 reg3 state) 
    = read_static_memory  address state\<close>

    \<open>read_dynamic_memory  address (BITWISE_XOR reg1 reg2 reg3 state) 
...
lemma (in Ironbark_world) write_register_BITWISE_XOR_same:
  assumes
    ...
    \<open>check_read_permission reg3\<close>
  shows
    \<open>write_register reg1 value (BITWISE_XOR reg1 reg2 reg3 state) 
...
text\<open>The following are cases of BITWISE\_XOR over the initial state.\<close>
...
lemma (in Ironbark_world) read_flag_BITWISE_XOR_initial_state:
  ...
  shows
    \<open>get_end_jump   (BITWISE_XOR reg1 reg2 reg3 start_state) = 0\<close>
    \<open>get_end_call   (BITWISE_XOR reg1 reg2 reg3 start_state) = 0\<close>
    \<open>get_end_return (BITWISE_XOR reg1 reg2 reg3 start_state) = 0\<close>
    \<open>get_halt       (BITWISE_XOR reg1 reg2 reg3 start_state) = 0\<close>
    \<open>get_error      (BITWISE_XOR reg1 reg2 reg3 start_state) = 0\<close>
  using assms
...
lemma (in Ironbark_world) common_flags_BITWISE_XOR_initial_state:
  ...
  defines
    \<open>start_state \<equiv> initial_state\<lparr>program_memory := program\<rparr>\<close>
  shows
    \<open>typical_flags (BITWISE_XOR reg1 reg2 reg3 start_state)\<close>
  using assms
...
lemma (in Ironbark_world) read_memory_BITWISE_XOR_initial_state:
  ...
  defines
    \<open>start_state \<equiv> initial_state\<lparr>program_memory := program\<rparr>\<close>
  shows
    \<open>read_program_memory  address (BITWISE_XOR reg1 reg2 reg3 start_state) 
    = read_program_memory address start_state\<close>

    \<open>read_call_memory     address (BITWISE_XOR reg1 reg2 reg3 start_state) 
    = read_call_memory    address start_state\<close>

    \<open>read_static_memory   address (BITWISE_XOR reg1 reg2 reg3 start_state) 
    = read_static_memory  address start_state\<close>

    \<open>read_dynamic_memory  address (BITWISE_XOR reg1 reg2 reg3 start_state) 
...
lemma (in Ironbark_world) read_ip_BITWISE_XOR_initial_state:
  ...
  defines
    \<open>start_state \<equiv> initial_state\<lparr>program_memory := program\<rparr>\<close>
  shows
    \<open>read_register instruction_pointer_ref (BITWISE_XOR reg1 reg2 reg3 start_state) 
