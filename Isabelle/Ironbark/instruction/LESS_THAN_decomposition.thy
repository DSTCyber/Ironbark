...
subsubsection \<open>Decomposition Rules Over LESS\_THAN\<close>

theory LESS_THAN_decomposition
...
of decomposing the \<open>LESS_THAN\<close> instruction. 
...
lemma (in Ironbark_world) LESS_THAN_decomp_state:
  ...
  shows
    \<open>LESS_THAN reg1 reg2 reg3 state = 
      state
      \<lparr>
        register_state := 
