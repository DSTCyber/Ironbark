...
subsubsection \<open>Decomposition Rules Over STORE\_STATIC\_DATA\<close>

theory STORE_STATIC_DATA_decomposition
...
decomposing the store static data instruction. The three lemmas are the `special' case 
...
Isabelle/HOL (`state'). The second is a decomposition that is a mix of native 
...
lemma (in Ironbark_world) STORE_STATIC_DATA_decomp_mixed:
  assumes
    \<open>typical_flags state\<close>
    \<open>check_read_permission reg1\<close>
    \<open>check_read_permission reg2\<close>
