...
subsubsection \<open>Simplification Rules Over LOAD\_DYNAMIC\_DATA\<close>

theory LOAD_DYNAMIC_DATA_simps
...
lemma (in Ironbark_world) flag_LOAD_DYNAMIC_DATA:
  ...
  shows
    \<open>flag_state (LOAD_DYNAMIC_DATA reg1 reg2 state) = flag_state state\<close>
  using assms
  by(simp add: LOAD_DYNAMIC_DATA_decomp_mixed)

lemma (in Ironbark_world) memory_LOAD_DYNAMIC_DATA:
  \<open>program_memory (LOAD_DYNAMIC_DATA reg1 reg2 state) = program_memory  state\<close>
  \<open>call_memory    (LOAD_DYNAMIC_DATA reg1 reg2 state) = call_memory     state\<close>
  \<open>static_memory  (LOAD_DYNAMIC_DATA reg1 reg2 state) = static_memory   state\<close>
  \<open>dynamic_memory (LOAD_DYNAMIC_DATA reg1 reg2 state) = dynamic_memory  state\<close>
  \<open>input_memory   (LOAD_DYNAMIC_DATA reg1 reg2 state) = input_memory    state\<close>
  \<open>output_memory  (LOAD_DYNAMIC_DATA reg1 reg2 state) = output_memory   state\<close>
...
lemma (in Ironbark_world) read_flag_LOAD_DYNAMIC_DATA:
  assumes
    ...
    \<open>check_read_permission reg2\<close>
  shows
    \<open>get_end_jump   (LOAD_DYNAMIC_DATA reg1 reg2 state) = 0\<close>
    \<open>get_end_call   (LOAD_DYNAMIC_DATA reg1 reg2 state) = 0\<close>
    \<open>get_end_return (LOAD_DYNAMIC_DATA reg1 reg2 state) = 0\<close>
    \<open>get_halt       (LOAD_DYNAMIC_DATA reg1 reg2 state) = 0\<close>
    \<open>get_error      (LOAD_DYNAMIC_DATA reg1 reg2 state) = 0\<close>
  using assms
...
lemma (in Ironbark_world) common_flags_LOAD_DYNAMIC_DATA:
  assumes
    ...
    \<open>check_read_permission reg2\<close>
  shows
    \<open>typical_flags (LOAD_DYNAMIC_DATA reg1 reg2 state)\<close>
  using assms
...
lemma (in Ironbark_world) read_register_LOAD_DYNAMIC_DATA_diff:
  ...
  shows
    \<open>read_register reg1 (LOAD_DYNAMIC_DATA reg2 reg3 state) = read_register reg1 state\<close>
  using assms
...
lemma (in Ironbark_world) read_register_LOAD_DYNAMIC_DATA_same:
  assumes
    ...
    \<open>check_read_permission reg2\<close>
  shows
    \<open>read_register reg1 (LOAD_DYNAMIC_DATA reg1 reg2 state) 
    = read_dynamic_memory (read_register reg2 state) state\<close>
  using assms
...
lemma (in Ironbark_world) read_ip_LOAD_DYNAMIC_DATA:
  assumes
    ...
    \<open>check_read_permission reg2\<close>
  shows
    \<open>read_register instruction_pointer_ref (LOAD_DYNAMIC_DATA reg1 reg2 state) 
...
lemma (in Ironbark_world) read_cycles_LOAD_DYNAMIC_DATA:
  assumes
    ...
    \<open>check_read_permission reg2\<close>
  shows
    \<open>read_register cycles_register_ref (LOAD_DYNAMIC_DATA reg1 reg2 state) 
...
lemma (in Ironbark_world) read_last_ip_LOAD_DYNAMIC_DATA:
  assumes
    ...
    \<open>check_read_permission reg2\<close>
  shows
    \<open>read_register last_instruction_pointer_ref (LOAD_DYNAMIC_DATA reg1 reg2 state) 
    = (read_register instruction_pointer_ref state)\<close>
  using assms
  by(simp add: LOAD_DYNAMIC_DATA_decomp_manipulation state_manipulation_simps)

lemma (in Ironbark_world) read_memory_LOAD_DYNAMIC_DATA:
  \<open>read_program_memory  address (LOAD_DYNAMIC_DATA reg1 reg2 state) 
  = read_program_memory address state\<close>

  \<open>read_call_memory     address (LOAD_DYNAMIC_DATA reg1 reg2 state) 
  = read_call_memory    address state\<close>

  \<open>read_static_memory   address (LOAD_DYNAMIC_DATA reg1 reg2 state) 
  = read_static_memory  address state\<close>

  \<open>read_dynamic_memory  address (LOAD_DYNAMIC_DATA reg1 reg2 state) 
  = read_dynamic_memory address state\<close>
  using memory_LOAD_DYNAMIC_DATA
  by(simp_all add: state_manipulation_decomp)

lemma (in Ironbark_world) write_register_LOAD_DYNAMIC_DATA_same:
  assumes
    \<open>typical_flags state\<close>
    \<open>check_write_permission reg1\<close>
    \<open>check_read_permission reg2\<close>
  shows
    \<open>write_register reg1 value (LOAD_DYNAMIC_DATA reg1 reg2 state) 
    = write_register 
        reg1 
        value 
...
text\<open>The following are lemmas of LOAD\_DYNAMIC\_DATA on the initial state.\<close>

lemma (in Ironbark_world) read_flag_LOAD_DYNAMIC_DATA_initial_state:
  fixes
  ...
  shows
    \<open>get_end_jump   (LOAD_DYNAMIC_DATA reg1 reg2 start_state) = 0\<close>
    \<open>get_end_call   (LOAD_DYNAMIC_DATA reg1 reg2 start_state) = 0\<close>
    \<open>get_end_return (LOAD_DYNAMIC_DATA reg1 reg2 start_state) = 0\<close>
    \<open>get_halt       (LOAD_DYNAMIC_DATA reg1 reg2 start_state) = 0\<close>
    \<open>get_error      (LOAD_DYNAMIC_DATA reg1 reg2 start_state) = 0\<close>
  using assms
  by(simp_all add: read_flag_LOAD_DYNAMIC_DATA initial_state_simps)

lemma (in Ironbark_world) common_flags_LOAD_DYNAMIC_DATA_initial_state:
  fixes
    program :: \<open>(64 word, 96 word) map\<close>
  assumes
    \<open>check_write_permission reg1\<close>
  ...
  defines
    \<open>start_state \<equiv> initial_state\<lparr>program_memory := program\<rparr>\<close>
  shows
    \<open>typical_flags (LOAD_DYNAMIC_DATA reg1 reg2 start_state)\<close>
  using assms
...
lemma (in Ironbark_world) read_memory_LOAD_DYNAMIC_DATA_initial_state:
  ...
  defines
    \<open>start_state \<equiv> initial_state\<lparr>program_memory := program\<rparr>\<close>
  shows
    \<open>read_program_memory  address (LOAD_DYNAMIC_DATA reg1 reg2 start_state) 
    = read_program_memory address start_state\<close>

    \<open>read_call_memory     address (LOAD_DYNAMIC_DATA reg1 reg2 start_state) 
    = read_call_memory    address start_state\<close>

    \<open>read_static_memory   address (LOAD_DYNAMIC_DATA reg1 reg2 start_state) 
    = read_static_memory  address start_state\<close>

    \<open>read_dynamic_memory  address (LOAD_DYNAMIC_DATA reg1 reg2 start_state) 
...
lemma (in Ironbark_world) read_ip_LOAD_DYNAMIC_DATA_initial_state:
  ...
  defines
    \<open>start_state \<equiv> initial_state\<lparr>program_memory := program\<rparr>\<close>
  shows
    \<open>read_register instruction_pointer_ref (LOAD_DYNAMIC_DATA reg1 reg2 start_state) 
