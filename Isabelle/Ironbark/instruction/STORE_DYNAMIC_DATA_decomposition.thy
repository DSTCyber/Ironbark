...
subsubsection \<open>Decomposition Rules Over STORE\_DYNAMIC\_DATA\<close>

theory STORE_DYNAMIC_DATA_decomposition
...
decomposing the store dynamic data instruction. The three lemmas are the `special' 
...
lemma (in Ironbark_world) STORE_DYNAMIC_DATA_decomp_state:
  ...
  shows
    \<open>STORE_DYNAMIC_DATA reg1 reg2 state = 
      state
      \<lparr>
        dynamic_memory := 
