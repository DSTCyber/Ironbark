...
subsubsection \<open>Decomposition Rules Over SUBTRACT\<close>

theory SUBTRACT_decomposition
...
decomposing the \<open>SUBTRACT\<close> instruction. The three lemmas are the `special' case where 
we assume the guards of the instruction will pass.
...
lemma (in Ironbark_world) SUBTRACT_decomp_state:
  ...
  shows
    \<open>SUBTRACT reg1 reg2 reg3 state = 
      state
      \<lparr>
        register_state := 
          ...
          (
            reg1 
              \<mapsto> the (register_state state reg2) - the (register_state state reg3),
