...
subsubsection \<open>Simplification Rules Over SHIFT\_LEFT\<close>

theory SHIFT_LEFT_simps
...
the \<open>SHIFT_LEFT\<close> instruction. 
...
lemma (in Ironbark_world) flag_SHIFT_LEFT:
  ...
  shows
    \<open>flag_state (SHIFT_LEFT reg1 reg2 reg3 state) = flag_state state\<close>
  using assms
  by(simp add: SHIFT_LEFT_decomp_mixed)

lemma (in Ironbark_world) memory_SHIFT_LEFT:
  \<open>program_memory (SHIFT_LEFT reg1 reg2 reg3 state) = program_memory  state\<close>
  \<open>call_memory    (SHIFT_LEFT reg1 reg2 reg3 state) = call_memory     state\<close>
  \<open>static_memory  (SHIFT_LEFT reg1 reg2 reg3 state) = static_memory   state\<close>
  \<open>dynamic_memory (SHIFT_LEFT reg1 reg2 reg3 state) = dynamic_memory  state\<close>
  \<open>input_memory   (SHIFT_LEFT reg1 reg2 reg3 state) = input_memory    state\<close>
  \<open>output_memory  (SHIFT_LEFT reg1 reg2 reg3 state) = output_memory   state\<close>
...
lemma (in Ironbark_world) read_flag_SHIFT_LEFT:
  assumes
    ...
    \<open>check_read_permission reg3\<close>
  shows
    \<open>get_end_jump   (SHIFT_LEFT reg1 reg2 reg3 state) = 0\<close>
    \<open>get_end_call   (SHIFT_LEFT reg1 reg2 reg3 state) = 0\<close>
    \<open>get_end_return (SHIFT_LEFT reg1 reg2 reg3 state) = 0\<close>
    \<open>get_halt       (SHIFT_LEFT reg1 reg2 reg3 state) = 0\<close>
    \<open>get_error      (SHIFT_LEFT reg1 reg2 reg3 state) = 0\<close>
  using assms
...
lemma (in Ironbark_world) common_flags_SHIFT_LEFT:
  assumes
    ...
    \<open>check_read_permission reg3\<close>
  shows
    \<open>typical_flags (SHIFT_LEFT reg1 reg2 reg3 state)\<close>
  using assms
...
lemma (in Ironbark_world) read_register_SHIFT_LEFT_diff:
  ...
  shows
    \<open>read_register regID (SHIFT_LEFT reg1 reg2 reg3 state) = read_register regID state\<close>
  using assms
...
lemma (in Ironbark_world) read_register_SHIFT_LEFT_same:
  assumes
    ...
    \<open>check_read_permission reg3\<close>
  shows
    \<open>read_register reg1 (SHIFT_LEFT reg1 reg2 reg3 state) 
    = push_bit (unat (read_register reg3 state)) (read_register reg2 state)\<close>
  using assms
  by(simp add: instruction_impl_defs state_manipulation_simps)
...
lemma (in Ironbark_world) read_ip_SHIFT_LEFT:
  assumes
    ...
    \<open>check_read_permission reg3\<close>
  shows
    \<open>read_register instruction_pointer_ref (SHIFT_LEFT reg1 reg2 reg3 state) 
...
lemma (in Ironbark_world) read_cycles_SHIFT_LEFT:
  assumes
    ...
    \<open>check_read_permission reg3\<close>
  shows
    \<open>read_register cycles_register_ref (SHIFT_LEFT reg1 reg2 reg3 state) 
...
lemma (in Ironbark_world) read_last_ip_SHIFT_LEFT:
  assumes
    ...
    \<open>check_read_permission reg3\<close>
  shows
    \<open>read_register last_instruction_pointer_ref (SHIFT_LEFT reg1 reg2 reg3 state) 
    = (read_register instruction_pointer_ref state)\<close>
  using assms
  by(simp add: instruction_impl_defs state_manipulation_simps)

lemma (in Ironbark_world) read_memory_SHIFT_LEFT:
  \<open>read_program_memory  address (SHIFT_LEFT reg1 reg2 reg3 state) 
  = read_program_memory address state\<close>

  \<open>read_call_memory     address (SHIFT_LEFT reg1 reg2 reg3 state) 
  = read_call_memory    address state\<close>

  \<open>read_static_memory   address (SHIFT_LEFT reg1 reg2 reg3 state) 
  = read_static_memory  address state\<close>

  \<open>read_dynamic_memory  address (SHIFT_LEFT reg1 reg2 reg3 state) 
...
lemma (in Ironbark_world) write_register_SHIFT_LEFT_same:
  assumes
    ...
    \<open>check_read_permission reg3\<close>
  shows
    \<open>write_register reg1 value (SHIFT_LEFT reg1 reg2 reg3 state) = 
      write_register 
        reg1 
        value 
...
text\<open>The following are cases of SHIFT\_LEFT over the initial state.\<close>
...
lemma (in Ironbark_world) read_flag_SHIFT_LEFT_initial_state:
  ...
  shows
    \<open>get_end_jump   (SHIFT_LEFT reg1 reg2 reg3 start_state) = 0\<close>
    \<open>get_end_call   (SHIFT_LEFT reg1 reg2 reg3 start_state) = 0\<close>
    \<open>get_end_return (SHIFT_LEFT reg1 reg2 reg3 start_state) = 0\<close>
    \<open>get_halt       (SHIFT_LEFT reg1 reg2 reg3 start_state) = 0\<close>
    \<open>get_error      (SHIFT_LEFT reg1 reg2 reg3 start_state) = 0\<close>
  using assms
...
lemma (in Ironbark_world) common_flags_SHIFT_LEFT_initial_state:
  ...
  defines
    \<open>start_state \<equiv> initial_state\<lparr>program_memory := program\<rparr>\<close>
  shows
    \<open>typical_flags (SHIFT_LEFT reg1 reg2 reg3 start_state)\<close>
  using assms
...
lemma (in Ironbark_world) read_memory_SHIFT_LEFT_initial_state:
  ...
  defines
    \<open>start_state \<equiv> initial_state\<lparr>program_memory := program\<rparr>\<close>
  shows
    \<open>read_program_memory  address (SHIFT_LEFT reg1 reg2 reg3 start_state) 
    = read_program_memory address start_state\<close>

    \<open>read_call_memory     address (SHIFT_LEFT reg1 reg2 reg3 start_state) 
    = read_call_memory    address start_state\<close>

    \<open>read_static_memory   address (SHIFT_LEFT reg1 reg2 reg3 start_state) 
    = read_static_memory  address start_state\<close>

    \<open>read_dynamic_memory  address (SHIFT_LEFT reg1 reg2 reg3 start_state) 
...
lemma (in Ironbark_world) read_last_ip_SHIFT_LEFT_initial_state:
  ...
  defines
    \<open>start_state \<equiv> initial_state\<lparr>program_memory := program\<rparr>\<close>
  shows
    \<open>read_register last_instruction_pointer_ref (SHIFT_LEFT reg1 reg2 reg3 start_state) 
...
lemma (in Ironbark_world) read_ip_SHIFT_LEFT_initial_state:
  ...
  defines
    \<open>start_state \<equiv> initial_state\<lparr>program_memory := program\<rparr>\<close>
  shows
    \<open>read_register instruction_pointer_ref (SHIFT_LEFT reg1 reg2 reg3 start_state) 
