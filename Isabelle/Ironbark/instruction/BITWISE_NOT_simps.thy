...
subsubsection \<open>Simplification Rules Over BITWISE\_NOT\<close>

theory BITWISE_NOT_simps
...
lemma (in Ironbark_world) flag_BITWISE_NOT:
  ...
  shows
    \<open>flag_state (BITWISE_NOT reg1 reg2 state) = flag_state state\<close>
  using assms
  by(simp add: BITWISE_NOT_decomp_mixed)

lemma (in Ironbark_world) memory_BITWISE_NOT:
  shows
    \<open>program_memory (BITWISE_NOT reg1 reg2 state) = program_memory  state\<close>
    \<open>call_memory    (BITWISE_NOT reg1 reg2 state) = call_memory     state\<close>
    \<open>static_memory  (BITWISE_NOT reg1 reg2 state) = static_memory   state\<close>
    \<open>dynamic_memory (BITWISE_NOT reg1 reg2 state) = dynamic_memory  state\<close>
    \<open>input_memory   (BITWISE_NOT reg1 reg2 state) = input_memory    state\<close>
    \<open>output_memory  (BITWISE_NOT reg1 reg2 state) = output_memory   state\<close>
...
lemma (in Ironbark_world) read_flag_BITWISE_NOT:
  assumes
    ...
    \<open>check_read_permission reg2\<close>
  shows
    \<open>get_end_jump   (BITWISE_NOT reg1 reg2 state) = 0\<close>
    \<open>get_end_call   (BITWISE_NOT reg1 reg2 state) = 0\<close>
    \<open>get_end_return (BITWISE_NOT reg1 reg2 state) = 0\<close>
    \<open>get_halt       (BITWISE_NOT reg1 reg2 state) = 0\<close>
    \<open>get_error      (BITWISE_NOT reg1 reg2 state) = 0\<close>
  using assms
...
lemma (in Ironbark_world) common_flags_BITWISE_NOT:
  assumes
    ...
    \<open>check_read_permission reg2\<close>
  shows
    \<open>typical_flags (BITWISE_NOT reg1 reg2 state)\<close>
  using assms
...
lemma (in Ironbark_world) read_register_BITWISE_NOT_diff:
  ...
  shows
    \<open>read_register reg1 (BITWISE_NOT reg2 reg3 state) = read_register reg1 state\<close>
  using assms
...
lemma (in Ironbark_world) read_register_BITWISE_NOT_same:
  ...
  assumes
    ...
    \<open>check_read_permission reg2\<close>
  shows
    \<open>read_register reg1 (BITWISE_NOT reg1 reg2 state) = NOT (read_register reg2 state)\<close>
  using assms
...
lemma (in Ironbark_world) read_ip_BITWISE_NOT:
  assumes
    ...
    \<open>check_read_permission reg2\<close>
  shows
    \<open>read_register instruction_pointer_ref (BITWISE_NOT reg1 reg2 state) 
...
lemma (in Ironbark_world) read_cycles_BITWISE_NOT:
  assumes
    ...
    \<open>check_read_permission reg2\<close>
  shows
    \<open>read_register cycles_register_ref (BITWISE_NOT reg1 reg2 state) 
...
lemma (in Ironbark_world) read_last_ip_BITWISE_NOT:
  assumes
    ...
    \<open>check_read_permission reg2\<close>
  shows
    \<open>read_register last_instruction_pointer_ref (BITWISE_NOT reg1 reg2 state) 
...
lemma (in Ironbark_world) read_memory_BITWISE_NOT:
  shows
    \<open>read_program_memory  address (BITWISE_NOT reg1 reg2 state) 
    = read_program_memory address state\<close>

    \<open>read_call_memory     address (BITWISE_NOT reg1 reg2 state) 
    = read_call_memory    address state\<close>

    \<open>read_static_memory   address (BITWISE_NOT reg1 reg2 state) 
    = read_static_memory  address state\<close>

    \<open>read_dynamic_memory  address (BITWISE_NOT reg1 reg2 state) 
...
lemma (in Ironbark_world) write_register_BITWISE_NOT_same:
  assumes
    ...
    \<open>check_read_permission reg2\<close>
  shows
    \<open>write_register reg1 value (BITWISE_NOT reg1 reg2 state) 
...
text\<open>The following are lemmas of BITWISE\_NOT on the initial state.\<close>
...
lemma (in Ironbark_world) read_flag_BITWISE_NOT_initial_state:
  ...
  shows
    \<open>get_end_jump   (BITWISE_NOT reg1 reg2 start_state) = 0\<close>
    \<open>get_end_call   (BITWISE_NOT reg1 reg2 start_state) = 0\<close>
    \<open>get_end_return (BITWISE_NOT reg1 reg2 start_state) = 0\<close>
    \<open>get_halt       (BITWISE_NOT reg1 reg2 start_state) = 0\<close>
    \<open>get_error      (BITWISE_NOT reg1 reg2 start_state) = 0\<close>
  using assms
  by(simp_all add: read_flag_BITWISE_NOT initial_state_simps)
...
lemma (in Ironbark_world) common_flags_BITWISE_NOT_initial_state:
  ...
  defines
    \<open>start_state \<equiv> initial_state\<lparr>program_memory := program\<rparr>\<close>
  shows
    \<open>typical_flags (BITWISE_NOT reg1 reg2 start_state)\<close>
  using assms
...
lemma (in Ironbark_world) read_memory_BITWISE_NOT_initial_state:
  ...
  defines
    \<open>start_state \<equiv> initial_state\<lparr>program_memory := program\<rparr>\<close>
  shows
    \<open>read_program_memory  address (BITWISE_NOT reg1 reg2 start_state) 
    = read_program_memory address start_state\<close>

    \<open>read_call_memory     address (BITWISE_NOT reg1 reg2 start_state) 
    = read_call_memory    address start_state\<close>

    \<open>read_static_memory   address (BITWISE_NOT reg1 reg2 start_state) 
    = read_static_memory  address start_state\<close>

    \<open>read_dynamic_memory  address (BITWISE_NOT reg1 reg2 start_state) 
...
lemma (in Ironbark_world) read_last_ip_BITWISE_NOT_initial_state:
  ...
  defines
    \<open>start_state \<equiv> initial_state\<lparr>program_memory := program\<rparr>\<close>
  shows
    \<open>read_register last_instruction_pointer_ref (BITWISE_NOT reg1 reg2 start_state) 
...
lemma (in Ironbark_world) read_ip_BITWISE_NOT_initial_state:
  ...
  defines
    \<open>start_state \<equiv> initial_state\<lparr>program_memory := program\<rparr>\<close>
  shows
    \<open>read_register instruction_pointer_ref (BITWISE_NOT reg1 reg2 start_state) 
