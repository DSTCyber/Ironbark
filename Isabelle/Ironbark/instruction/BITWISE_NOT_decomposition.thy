...
subsubsection \<open>Decomposition Rules Over BITWISE\_NOT\<close>

theory BITWISE_NOT_decomposition
...
of decomposing the \<open>BITWISE_NOT\<close> instruction. 
...
lemma (in Ironbark_world) BITWISE_NOT_decomp_state:
  includes 
  ...
  shows
    \<open>BITWISE_NOT reg1 reg2 state = 
      state
      \<lparr>
        register_state := 
          ...
          (
            reg1 
              \<mapsto> NOT (the (register_state state reg2)),
    ...
    \<open>BITWISE_NOT reg1 reg2 state = 
      ...
      \<lparr>
        register_state := 
          ...
          (
            reg1 
              \<mapsto> NOT (read_register reg2 state),
            last_instruction_pointer_ref 
