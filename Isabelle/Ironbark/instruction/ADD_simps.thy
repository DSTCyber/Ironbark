...
subsubsection \<open>Simplification Rules Over ADD\<close>
...
text \<open>As with most instructions, we provide various simplification rules for the \<open>ADD\<close> 
...
lemma (in Ironbark_world) flag_ADD:
  assumes
  ...
  shows
    \<open>flag_state (ADD reg1 reg2 reg3 state) = flag_state state\<close>
  using assms
  by(simp add: ADD_decomp_mixed)

lemma (in Ironbark_world) memory_ADD:
  \<open>program_memory (ADD reg1 reg2 reg3 state) = program_memory state\<close>
  \<open>call_memory    (ADD reg1 reg2 reg3 state) = call_memory    state\<close>
  \<open>static_memory  (ADD reg1 reg2 reg3 state) = static_memory  state\<close>
  \<open>dynamic_memory (ADD reg1 reg2 reg3 state) = dynamic_memory state\<close>
  \<open>input_memory   (ADD reg1 reg2 reg3 state) = input_memory   state\<close>
  \<open>output_memory  (ADD reg1 reg2 reg3 state) = output_memory  state\<close>
  by(simp_all add: instruction_impl_defs state_manipulation_simps)

lemma (in Ironbark_world) read_flag_ADD:
  assumes
    \<open>typical_flags state\<close>
    \<open>check_write_permission reg1\<close>
    \<open>check_read_permission reg2\<close>
    \<open>check_read_permission reg3\<close>
  shows
    \<open>get_end_jump   (ADD reg1 reg2 reg3 state) = 0\<close>
    \<open>get_end_call   (ADD reg1 reg2 reg3 state) = 0\<close>
    \<open>get_end_return (ADD reg1 reg2 reg3 state) = 0\<close>
    \<open>get_halt       (ADD reg1 reg2 reg3 state) = 0\<close>
    \<open>get_error      (ADD reg1 reg2 reg3 state) = 0\<close>
  using assms
...
lemma (in Ironbark_world) common_flags_ADD:
  assumes
    ...
    \<open>check_read_permission reg3\<close>
  shows
    \<open>typical_flags (ADD reg1 reg2 reg3 state)\<close>
  using assms
...
lemma (in Ironbark_world) read_register_ADD_diff:
  assumes
    ...
    shows
    \<open>read_register regID (ADD reg1 reg2 reg3 state) = read_register regID state\<close>
  using assms
...
lemma (in Ironbark_world) read_register_ADD_same:
  assumes
    ...
    \<open>check_read_permission reg3\<close>
  shows
    \<open>read_register reg1 (ADD reg1 reg2 reg3 state) 
...
lemma (in Ironbark_world) read_ip_ADD:
  assumes
    ...
    \<open>check_read_permission reg3\<close>
  shows
    \<open>read_register    instruction_pointer_ref (ADD reg1 reg2 reg3 state) 
...
lemma (in Ironbark_world) read_cycles_ADD:
  assumes
    ...
    \<open>check_read_permission reg3\<close>
  shows
    \<open>read_register cycles_register_ref (ADD reg1 reg2 reg3 state) 
...
lemma (in Ironbark_world) read_last_ip_ADD:
  assumes
    ...
    \<open>check_read_permission reg3\<close>
  shows
    \<open>read_register    last_instruction_pointer_ref  (ADD reg1 reg2 reg3 state) 
    = (read_register  instruction_pointer_ref       state)\<close>
  using assms
  by(simp add: instruction_impl_defs state_manipulation_simps)

lemma (in Ironbark_world) read_memory_ADD:
  \<open>read_program_memory  address (ADD reg1 reg2 reg3 state) 
  = read_program_memory address state\<close>
  \<open>read_call_memory     address (ADD reg1 reg2 reg3 state) 
  = read_call_memory    address state\<close>
  \<open>read_static_memory   address (ADD reg1 reg2 reg3 state) 
  = read_static_memory  address state\<close>
  \<open>read_dynamic_memory  address (ADD reg1 reg2 reg3 state) 
...
lemma (in Ironbark_world) write_register_ADD_same:
  assumes
    ...
    \<open>check_read_permission reg3\<close>
  shows
    \<open>write_register reg1 value (ADD reg1 reg2 reg3 state) 
...
text\<open>The following are cases of ADD over the initial state.\<close>
...
lemma (in Ironbark_world) read_flag_ADD_initial_state:
  ...
  shows
    \<open>get_end_jump   (ADD reg1 reg2 reg3 start_state) = 0\<close>
    \<open>get_end_call   (ADD reg1 reg2 reg3 start_state) = 0\<close>
    \<open>get_end_return (ADD reg1 reg2 reg3 start_state) = 0\<close>
    \<open>get_halt       (ADD reg1 reg2 reg3 start_state) = 0\<close>
    \<open>get_error      (ADD reg1 reg2 reg3 start_state) = 0\<close>
  using assms
...
lemma (in Ironbark_world) common_flags_ADD_initial_state:
  ...
  defines
    \<open>start_state \<equiv> initial_state\<lparr>program_memory := program\<rparr>\<close>
  shows
    \<open>typical_flags (ADD reg1 reg2 reg3 start_state)\<close>
  using assms
...
lemma (in Ironbark_world) read_memory_ADD_initial_state:
  ...
  defines
    \<open>start_state \<equiv> initial_state\<lparr>program_memory := program\<rparr>\<close>
  shows
    \<open>read_program_memory  address (ADD reg1 reg2 reg3 start_state) 
    = read_program_memory address start_state\<close>

    \<open>read_call_memory     address (ADD reg1 reg2 reg3 start_state) 
    = read_call_memory    address start_state\<close>

    \<open>read_static_memory   address (ADD reg1 reg2 reg3 start_state) 
    = read_static_memory  address start_state\<close>

    \<open>read_dynamic_memory  address (ADD reg1 reg2 reg3 start_state) 
...
lemma (in Ironbark_world) read_last_ip_ADD_initial_state:
  ...
  defines
    \<open>start_state \<equiv> initial_state\<lparr>program_memory := program\<rparr>\<close>
  shows
    \<open>read_register  last_instruction_pointer_ref  (ADD reg1 reg2 reg3 start_state) 
...
lemma (in Ironbark_world) read_ip_ADD_initial_state:
  ...
  defines
    \<open>start_state \<equiv> initial_state\<lparr>program_memory := program\<rparr>\<close>
  shows
    \<open>read_register    instruction_pointer_ref (ADD reg1 reg2 reg3 start_state) 
