...
subsubsection \<open>Decomposition Rules Over BITWISE\_AND\<close>

theory BITWISE_AND_decomposition
...
decomposing the  \<open>BITWISE_AND\<close> instruction. The three lemmas are the `special' case 
...
lemma (in Ironbark_world) BITWISE_AND_decomp_state: 
  ...
  shows
    \<open>BITWISE_AND reg1 reg2 reg3 state = 
      state
      \<lparr>
        register_state := 
          ...
          (
            reg1 
              \<mapsto> the (register_state state reg2) AND the (register_state state reg3),
    ...
    \<open>BITWISE_AND reg1 reg2 reg3 state =
      ...
      \<lparr>
        register_state :=
          ...
          (
            reg1
              \<mapsto> (read_register reg2 state) AND (read_register reg3 state),
            last_instruction_pointer_ref
