...
subsubsection \<open>Simplification Rules Over STORE\_DYNAMIC\_DATA\<close>
...
lemma (in Ironbark_world) flag_STORE_DYNAMIC_DATA:
  ...
  shows
    \<open>flag_state (STORE_DYNAMIC_DATA reg1 reg2 state) = flag_state state\<close>
  using assms
  by(simp add: STORE_DYNAMIC_DATA_decomp_mixed)

lemma (in Ironbark_world) memory_STORE_DYNAMIC_DATA_same:
  \<open>program_memory (STORE_DYNAMIC_DATA reg1 reg2 state) = program_memory state\<close>
  \<open>call_memory    (STORE_DYNAMIC_DATA reg1 reg2 state) = call_memory    state\<close>
  \<open>static_memory  (STORE_DYNAMIC_DATA reg1 reg2 state) = static_memory  state\<close>
  \<open>input_memory   (STORE_DYNAMIC_DATA reg1 reg2 state) = input_memory   state\<close>
  \<open>output_memory  (STORE_DYNAMIC_DATA reg1 reg2 state) = output_memory  state\<close>
...
lemma (in Ironbark_world) memory_STORE_DYNAMIC_DATA_diff:
  assumes
    ...
    \<open>check_read_permission reg2\<close>
  shows
    \<open>dynamic_memory (STORE_DYNAMIC_DATA reg1 reg2 state) = 
      dynamic_memory 
...
lemma (in Ironbark_world) read_flag_STORE_DYNAMIC_DATA:
  assumes
    ...
    \<open>check_read_permission reg2\<close>
  shows
    \<open>get_end_jump   (STORE_DYNAMIC_DATA reg1 reg2 state) = 0\<close>
    \<open>get_end_call   (STORE_DYNAMIC_DATA reg1 reg2 state) = 0\<close>
    \<open>get_end_return (STORE_DYNAMIC_DATA reg1 reg2 state) = 0\<close>
    \<open>get_halt       (STORE_DYNAMIC_DATA reg1 reg2 state) = 0\<close>
    \<open>get_error      (STORE_DYNAMIC_DATA reg1 reg2 state) = 0\<close>
  using assms
...
lemma (in Ironbark_world) common_flags_STORE_DYNAMIC_DATA:
  assumes
    ...
    \<open>check_read_permission reg2\<close>
  shows
    \<open>typical_flags (STORE_DYNAMIC_DATA reg1 reg2 state)\<close>
  using assms
...
lemma (in Ironbark_world) read_register_STORE_DYNAMIC_DATA:
  ...
  shows
    \<open>read_register    regID (STORE_DYNAMIC_DATA reg1 reg2 state) 
...
lemma (in Ironbark_world) read_ip_STORE_DYNAMIC_DATA:
  assumes
    ...
    \<open>check_read_permission reg2\<close>
  shows
    \<open>read_register instruction_pointer_ref (STORE_DYNAMIC_DATA reg1 reg2 state) 
...
lemma (in Ironbark_world) read_cycles_STORE_DYNAMIC_DATA:
  assumes
    ...
    \<open>check_read_permission reg2\<close>
  shows
    \<open>read_register cycles_register_ref (STORE_DYNAMIC_DATA reg1 reg2 state) 
...
lemma (in Ironbark_world) read_last_ip_STORE_DYNAMIC_DATA:
  assumes
    ...
    \<open>check_read_permission reg2\<close>
  shows
    \<open>read_register last_instruction_pointer_ref (STORE_DYNAMIC_DATA reg1 reg2 state) 
    = (read_register instruction_pointer_ref state)\<close>
  using assms
  by(simp add: STORE_DYNAMIC_DATA_decomp_manipulation state_manipulation_simps)

lemma (in Ironbark_world) read_memory_STORE_DYNAMIC_DATA_same:
  \<open>read_program_memory  address (STORE_DYNAMIC_DATA reg1 reg2 state) 
  = read_program_memory address state\<close>

  \<open>read_call_memory     address (STORE_DYNAMIC_DATA reg1 reg2 state) 
  = read_call_memory    address state\<close>

  \<open>read_static_memory   address (STORE_DYNAMIC_DATA reg1 reg2 state) 
...
lemma (in Ironbark_world) read_memory_STORE_DYNAMIC_DATA_diff:
  ...
  shows
    \<open>read_dynamic_memory address (STORE_DYNAMIC_DATA reg1 reg2 state) 
...
next
  case False
  then have 
    \<open>STORE_DYNAMIC_DATA reg1 reg2 state = standard_error state\<close>
    by(simp add: instruction_impl_defs state_manipulation_simps, linarith)
  then show ?thesis 
    by(simp add: state_manipulation_simps)
qed

text\<open>The following are lemmas of STORE\_DYNAMIC\_DATA on the initial state.\<close>
...
lemma (in Ironbark_world) read_flag_STORE_DYNAMIC_DATA_initial_state:
  ...
  shows
    \<open>get_end_jump   (STORE_DYNAMIC_DATA reg1 reg2 start_state) = 0\<close>
    \<open>get_end_call   (STORE_DYNAMIC_DATA reg1 reg2 start_state) = 0\<close>
    \<open>get_end_return (STORE_DYNAMIC_DATA reg1 reg2 start_state) = 0\<close>
    \<open>get_halt       (STORE_DYNAMIC_DATA reg1 reg2 start_state) = 0\<close>
    \<open>get_error      (STORE_DYNAMIC_DATA reg1 reg2 start_state) = 0\<close>
  using assms
  by(simp_all add: read_flag_STORE_DYNAMIC_DATA initial_state_simps)
...
lemma (in Ironbark_world) common_flags_STORE_DYNAMIC_DATA_initial_state:
  ...
  defines
    \<open>start_state \<equiv> initial_state\<lparr>program_memory := program\<rparr>\<close>
  shows
    \<open>typical_flags (STORE_DYNAMIC_DATA reg1 reg2 start_state)\<close>
  using assms
...
lemma (in Ironbark_world) read_memory_STORE_DYNAMIC_DATA_initial_state:
  ...
  defines
    \<open>start_state \<equiv> initial_state\<lparr>program_memory := program\<rparr>\<close>
  shows
    \<open>read_program_memory  address (STORE_DYNAMIC_DATA reg1 reg2 start_state) 
    = read_program_memory address start_state\<close>

    \<open>read_call_memory     address (STORE_DYNAMIC_DATA reg1 reg2 start_state) 
    = read_call_memory    address start_state\<close>

    \<open>read_static_memory   address (STORE_DYNAMIC_DATA reg1 reg2 start_state) 
    = read_static_memory  address start_state\<close>
  using assms
  by(simp_all add: read_memory_STORE_DYNAMIC_DATA_same)
...
lemma (in Ironbark_world) read_last_ip_STORE_DYNAMIC_DATA_initial_state:
  ...
  using assms
  by(simp add: read_last_ip_STORE_DYNAMIC_DATA initial_state_simps)
...
lemma (in Ironbark_world) read_ip_STORE_DYNAMIC_DATA_initial_state:
  ...
  defines
    \<open>start_state \<equiv> initial_state\<lparr>program_memory := program\<rparr>\<close>
  shows
    \<open>read_register instruction_pointer_ref (STORE_DYNAMIC_DATA reg1 reg2 start_state) 
    = (read_register instruction_pointer_ref start_state) + 1\<close>
  using assms
  by(simp add: read_ip_STORE_DYNAMIC_DATA initial_state_simps)
