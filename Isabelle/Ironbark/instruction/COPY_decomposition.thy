...
subsubsection \<open>Decomposition Rules Over COPY\<close>

theory COPY_decomposition
...
decomposing the \<open>COPY\<close> instruction. The three lemmas are the `special' case where we 
    ...
    \<open>COPY reg1 reg2 state = 
      ...
      \<lparr>
        register_state := 
          ...
          (
            reg1 
              \<mapsto> the (register_state state reg2),
